...
async function processEmail(opts: ProcessEmailOptions): Promise<void> {
  ...
  // Check sender filtering (allowlist/blocklist)
  const filterMode = account.config.filterMode ?? "open";
  const senderLower = email.from.toLowerCase();
  
  // Always check blocklist first (applies in all modes)
  if (account.config.blockFrom?.length) {
    const isBlocked = account.config.blockFrom.some(pattern => {
      ...
      return senderLower === patternLower || senderLower.endsWith("@" + patternLower);
    });
    if (isBlocked) {
      log?.debug(`[${account.accountId}] Blocked sender: ${email.from}`);
      markMessageProcessed(email.messageId);
      await markAsRead(client, folder, email.uid);
      return;
    }
  ...
  if (filterMode === "allowlist") {
    const allowList = account.config.allowFrom ?? [];
    if (allowList.length === 0) {
      log?.debug(`[${account.accountId}] Allowlist mode but no addresses configured, skipping: ${email.from}`);
      markMessageProcessed(email.messageId);
      await markAsRead(client, folder, email.uid);
      return;
    }
    ...
    if (!isAllowed) {
      log?.debug(`[${account.accountId}] Sender not in allowlist: ${email.from}`);
      markMessageProcessed(email.messageId);
      await markAsRead(client, folder, email.uid);
      return;
    }
